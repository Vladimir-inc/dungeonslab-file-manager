...
// ключ всегда считаем из source/path: id элемента может нести панельный префикс ("2|data:...")
// и как ключ хранилища не годится
function storageKey(it) {
  return it.source != null && it.path != null ? keyFor(it.source, it.path) : it.id;
}

export function stampEntryFields(entries, items, patch) {
  const next = { ...entries };
  for (const it of items) {
    const key = storageKey(it);
