...
describe("stampEntryFields", () => {
  ...
  it("derives the storage key from source/path, ignoring pane-prefixed ids", () => {
    const prefixed = { ...item("data", "maps", "maps", true), id: "2|data:maps" };
    const next = stampEntryFields({}, [prefixed], { favorite: true });
    expect(Object.keys(next)).toEqual(["data:maps"]);
  });
